...
module Pubnub
  class Client
      ...
      if @env[:heartbeat]
        ...
        @env[:respirator] = EM.add_periodic_timer((@env[:heartbeat].to_i/2) - 1) do
          ...
          #   $logger.debug('Pubnub'){'Pubnub::Client#start_respirator | BUM'}
            EM.defer { heartbeat(:channel => @env[:subscriptions].get_channels, :group => subscribe.get_channel_groups ){ |e| $logger.debug('Pubnub::Client#start_respirator | bum') } }
          # end
    ...
    def start_origin_manager
      ...
      @origin_manager.start_current_origin_manager
    end
